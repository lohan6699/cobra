...
// ---------- Som ----------
let audioCtx = null;
let somLigado = localStorage.getItem('cobrinha-som') !== '0';
function atualizarBotaoSom() { btnSom.textContent = somLigado ? '🔊 Som: Ligado' : '🔇 Som: Desligado'; }
atualizarBotaoSom();
btnSom.addEventListener('click', () => {
  somLigado = !somLigado;
  localStorage.setItem('cobrinha-som', somLigado ? '1' : '0');
  atualizarBotaoSom();
});
...
function tocarSom(tipo) {
  ...
  } catch (e) { /* ambiente sem suporte a áudio */ }
}
...
let semParede = localStorage.getItem('cobrinha-sem-parede') === '1';
...
btnParede.addEventListener('click', () => {
  semParede = !semParede;
  localStorage.setItem('cobrinha-sem-parede', semParede ? '1' : '0');
  atualizarBotaoParede();
});
...
function renderizarLoja() {
  ...
  skins.forEach(skin => {
    ...
    if (equipada) {
      ...
      botao.onclick = () => { skinAtual = skin.id; localStorage.setItem('cobrinha-skin', skinAtual); tocarSom('compra'); renderizarLoja(); };
    } else {
...
let totalMacas = Number(localStorage.getItem('cobrinha-total-macas')) || 0;
...
function verificarBonusDiario() {
  ...
  const ultimo = localStorage.getItem('cobrinha-ultimo-bonus');
  if (ultimo === hoje) return;
  const ontem = new Date(Date.now() - 86400000).toDateString();
  let streak = Number(localStorage.getItem('cobrinha-streak')) || 0;
  streak = (ultimo === ontem) ? streak + 1 : 1;
  if (streak > 7) streak = 7;
  const bonus = 10 + streak * 5;
  moedas += bonus;
  salvarMoedas();
  localStorage.setItem('cobrinha-ultimo-bonus', hoje);
  localStorage.setItem('cobrinha-streak', streak);
...
function atualizar() {
  ...
  if (cabeca.x === comida.x && cabeca.y === comida.y) {
    ...
    moedas += 1;
    totalMacas += 1;
    localStorage.setItem('cobrinha-total-macas', totalMacas);
    elPontos.textContent = pontos;
    salvarMoedas();
    tocarSom('comer');
...
function fimDeJogo() {
  ...
  function passoExplosao() {
    ...
    quadros++;
    if (quadros < 45 && particulas.length > 0) requestAnimationFrame(passoExplosao);
    else mostrarTelaFim();
  }
  passoExplosao();
}
...
function mostrarTelaFim() {
  ...
  pontosFinal.textContent = `Você fez ${pontos} pontos (nível ${nivel})`;
